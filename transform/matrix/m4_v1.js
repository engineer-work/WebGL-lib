(function(root, factory) {  // eslint-disable-line
  ...
  // 🔒 FIX: axisRotation handles zero-length axis 
  function axisRotation_fix(axis, angle, dst) {
  dst = dst || new MatType(16);
  ...
  function quatFromRotationMatrix(m, dst) {
   ...
   function quatFromRotationMatrix_fix(m, dst) {
  dst = dst || new MatType(4);   // ✅ FIX
...
function transformVector_fix(m, v, dst) {
  dst = dst || new MatType(4);
...
function transformPoint_fix(m, v, dst) {
  dst = dst || new MatType(3);
...
function transformDirection_fix(m, v, dst) {
  dst = dst || new MatType(3);
...
function transformNormal_fix(m, v, dst) {
  dst = dst || new MatType(3);

  const inv = inverse(m);
  const invT = transpose(inv);

  return transformDirection(invT, v, dst);
}
  ...
  return {
    ...
    copy_fix: copy_fix,
axisRotation_fix   : axisRotation_fix,
transformVector_fix :   transformVector_fix,
transformPoint_fix:  transformPoint_fix,
transformDirection_fix : transformDirection_fix,
transformNormal_fix : transformNormal_fix,
quatFromRotationMatrix_fix, quatFromRotationMatrix_fix
  };
